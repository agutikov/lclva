...
The ONNX model file ships separately; downloaded via `tools/acva-models install silero-v5` (post-M7) to `${XDG_DATA_HOME}/acva/models/silero/silero_vad.onnx`.
...
- `scripts/download-vad.sh` pulls the v5.1.2 ONNX model. *(Post-M7: replaced by `tools/acva-models install silero-v5`; the registry alias is `silero-v5` under `models.vad` in `config/default.yaml`.)*
