...
#
# Override the destination root with ACVA_MODELS_DIR (the same env
# var compose reads). Total disk footprint of the defaults below:
# ~4.5 GB (LLM only — STT and TTS now go through Speaches).
...
#
# Post-M4B this script only handles LLM weights — Speaches handles its
# own STT and TTS model installs via POST /v1/models/{id}, driven by
